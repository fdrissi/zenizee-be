...
elseif ($_POST["type"] == "edit_user") {
        ...
        $table = "tbl_user";
        $field = [
            "name"   => mysqli_real_escape_string($evmulti, $_POST["name"]),
            "email"  => mysqli_real_escape_string($evmulti, $_POST["email"]),
            "mobile" => mysqli_real_escape_string($evmulti, $_POST["mobile"]),
            "wallet" => mysqli_real_escape_string($evmulti, $_POST["wallet"]),
...
elseif ($_POST["type"] == "edit_profile") {
        ...
        $adminRow = $evmulti->query("SELECT password FROM admin WHERE id=" . $id)->fetch_assoc();
        if (!$adminRow || $adminRow["password"] !== $currentPassword) {
            $returnArr = [
                "ResponseCode" => "200",
                "Result" => "false",
            ...
            $table = "admin";
            $escapedPassword = mysqli_real_escape_string($evmulti, $newPassword);
            $field = ["username" => $username, "password" => $escapedPassword];
            $where = "where id=" . $id;
            $h = new Event();
            $check = $h->evmultiupdateData($field, $table, $where);
            if ($check == 1) {
        ...
        $exists = $evmulti->query("SELECT id FROM admin WHERE username='" . $username . "' AND id!=" . $id)->num_rows;
        if ($exists > 0) {
            $returnArr = [
                "ResponseCode" => "200",
                "Result" => "false",
                "title" => "Username Already Exists!",
                "message" => "Another admin already uses this username.",
                "action" => "list_user.php",
            ];
            ...
            if (!empty($newPassword)) {
                $field["password"] = mysqli_real_escape_string($evmulti, $newPassword);
            }
            $table = "admin";
            $where = "where id=" . $id;
            $h = new Event();
            ...
            if ($check == 1) {
                ...
                $returnArr = [
                    ...
                    "Result" => "false",
                    "title" => "Update Failed!",
                    "message" => "Could not update admin. Please try again.",
                    "action" => "list_user.php",
                ];
            }
        }
    } elseif ($_POST["type"] == "create_admin") {
        $username = mysqli_real_escape_string($evmulti, $_POST["username"]);
        $password = $_POST["password"];
        $confirmPassword = $_POST["confirm_password"];

        // Check if username already exists
        $exists = $evmulti->query("SELECT id FROM admin WHERE username='" . $username . "'")->num_rows;
        if ($exists > 0) {
            $returnArr = [
                "ResponseCode" => "200",
                "Result" => "false",
            ...
            $escapedPassword = mysqli_real_escape_string($evmulti, $password);
            $h = new Event();
            $check = $h->evmultiinsertdata(
                ["username", "password"],
            ...
            if ($check) {
                ...
                $returnArr = [
                    ...
                    "message" => "Could not create admin. Please try again.",
                    "action" => "list_user.php",
                ];
            }
        }
