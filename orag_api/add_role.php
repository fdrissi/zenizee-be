...
if ($status == '' or $orag_id == '' or $manager_type == '' or $password == '' or $email == '') {
	...
	$check_record = $evmulti->query("select * from tbl_omanager where email='" . $email . "'")->num_rows;
	if($check_record !=0)
{
$returnArr = array(
        "ResponseCode" => "401",
...
$table = "tbl_omanager";
            $field_values = ["status","orag_id","manager_type","name","email","password"];
            $data_values = ["$status","$orag_id","$manager_type","$name","$email","$password"];
			...
			$returnArr    = array(
                "ResponseCode" => "200",
                "Result" => "true",
                "ResponseMsg" => "Role  Add Successfully"
            );
}	
	}
